...
t_vec3	ray_casted_color(t_data *data, int x, int y)
{
	...
	result = vec3_mult(first_hit_obj->proprieties.color,
			vec3_div_float(vec3_add(
					p_light->proprieties.color, a_light->proprieties.color),
				2));
	...
	n = fmax(a_light->proprieties.parameter,
			vec3_dot(first_hit_obj->hit_record.normal,
				vec3_normalize(vec3_sub(
						p_light->center,
						first_hit_obj->hit_record.hit_point))));
