...
/*   Updated: 2025/04/02 00:38:08 by jrakoton         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ambient_parser(t_ambient *ambient, char *scene_elt, t_scene *scene)
{
	...
	ambient->ratio = ft_atof(splitted_elt[1]);
	if (!is_in_range(ambient->ratio, 'r'))
		free_pars_error(scene, splitted_elt, "Ratio value should be in range of [0.0f, 0.1]");
