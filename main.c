...
void	create_obj(t_list *tmp, t_data *data)
{
	...
	if (obj->shape == CAMERA)
		create_camera(data, obj->center, vec3_unit(obj->normal), obj->diameter);
	else
	{
		prt = det_proprts(obj);
		if (obj->shape == SPHERE || obj->shape == POINT_LIGHT)
			...
			new_obj = create_plane(obj->center, vec3_unit(obj->normal));
		else if (obj->shape == CYLINDRE)
			new_obj = create_cylinder(obj->center,
					vec3_unit(obj->normal), obj->diameter, obj->height);
		else if (obj->shape == CONE)
			new_obj = create_cone(obj->center, vec3_unit(obj->normal),
